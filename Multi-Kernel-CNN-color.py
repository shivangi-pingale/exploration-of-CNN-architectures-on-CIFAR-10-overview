...
L2 = Convolutional3DLayer(kSize=3,numKernels=3) # 3 kernels
L3 = MaxPoolLayer(size=2, stride=2)
L4 = FlatteningLayer()
L5 = FullyConnectedLayer(sizeIn=675, sizeOut=10)
L6 = SoftmaxLayer()
L7 = CrossEntropy()
...
for e in range(epochs):    
    ...
    # Print loss after every 50 epochs
    if (e + 1) % 10 == 0:
...
# Save and display Cross Entropy Loss plot
plt.figure()
...
plt.grid()
plt.legend()
plt.savefig("Loss-color-3-kernel.png")
plt.show()
...
plt.grid()
plt.legend()
plt.savefig("Accuracy-color-3-kernel.png")
plt.show()
